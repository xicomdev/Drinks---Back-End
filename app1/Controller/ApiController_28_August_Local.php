 ...
 */
	...
	public function getUserProfile() {
		...
		if (!empty($this->request->data['user_id'])) {
			...
			if(!empty($resultset)){
				...
				$resultArray['status'] = true;
				$resultArray['data'] = $resultset;
				$resultArray['message'] = "user found";	
			}else{
				$resultArray = array();
				$resultArray['status'] = false;
				$resultArray['data'] = new stdClass();
				$resultArray['message'] = "user not found with this id";
			}
	...
	public function searchGroups() {
		...
		if(isset($this->request->data['age']) && !empty($this->request->data['age'])){
			//$params = array_merge($params,array('Group.age' => $this->request->data['']));	
			//$params = array_merge($params,array('Group.tags.age between ? and ?' => $this->request->data['age']));
			$age = explode(',', $this->request->data['age']);
			$min_age = $age[0];
			$max_age = $age[1];
			$params = array_merge($params,array('Group.tags.age' => $min_age));
		}
		//print_r($params); 
		//die;

		$resultset = $this->Group->find('all',array('conditions' =>$params));
		//print_r($resultset);
		if(!empty($resultset)){
